...
public class GridMovementHighlighter : MonoBehaviour
{
    ...
    bool IsValidMovePosition(Vector2Int gridPos, GameObject groundObject)
    {
        ...
        {
            // Also check other ground objects for this grid position
            return CheckOtherGroundObjects(gridPos);
        }
    ...
    bool CheckOtherGroundObjects(Vector2Int gridPos)
    {
        // Find all ground objects and check if this grid position is valid on any of them
        GameObject[] allObjects = FindObjectsByType<GameObject>(FindObjectsSortMode.None);
        ...
        foreach (GameObject obj in allObjects)
        {
            if (obj.layer == LayerMask.NameToLayer("Ground") && obj.GetComponent<Renderer>() != null)
            {
                ...
                {
                    // Check if not occupied and reachable
                    if (!placementManager.IsTileOccupied(obj, gridPos))
                    {
                        if (heightChecker == null || heightChecker.IsPositionReachable(gridPos, obj))
                        {
                            return true;
                        }
                    }
    ...
    void CreateMovementHighlight(Vector2Int gridPos, GameObject groundObject)
    {
        ...
        // Create material with our movement color - using Unlit shader to avoid lighting issues
        Material highlightMaterial = new Material(Shader.Find("Unlit/Color"));
        
        // Set the color directly - no transparency for now to test
        highlightMaterial.color = movementColor;
        ...
        if (enableDebugLogs && movementHighlights.Count <= 3) // Only log first few
        {
            Debug.Log($"Created highlight at {worldPos} for grid {gridPos} on {groundObject.name}");
        }
    }
