...
public class EnemySpawner : MonoBehaviour
{
    ...
    bool IsValidSpawnPosition(Vector2Int gridPos, GameObject groundObject)
    {
        ...
        // Check minimum distance from all existing units (player and enemy)
        if (!IsMinimumDistanceFromUnits(gridPos, groundObject))
        {
            return false;
        }
    ...
    bool IsMinimumDistanceFromUnits(Vector2Int gridPos, GameObject groundObject)
    {
        Vector3 worldPos = gridManager.GridToWorldPosition(gridPos, groundObject);
        
        // Check distance from all existing player units
