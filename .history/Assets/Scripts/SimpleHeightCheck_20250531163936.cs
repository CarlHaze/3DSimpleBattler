...
public class SimpleHeightCheck : MonoBehaviour
{
    ...
    bool CanPlaceHere(Vector3 position)
    {
        ...
        bool raycastHit = Physics.Raycast(rayStart, rayDirection, out hit, rayDistance, groundLayerMask);
        
        if (enableDebugLogs)
        {
            Debug.Log($"Raycast hit something: {raycastHit}");
        }
        ...
        else
        {
            if (enableDebugLogs)
            {
                Debug.Log($"NO HIT FOUND within {rayDistance} units - BLOCKING PLACEMENT");
            }
