...
public class GridMovementHighlighter : MonoBehaviour
{
    ...
    bool IsPositionOccupied(Vector2Int gridPos, GameObject groundObject)
    {
        ...
        // NEW: Check if position is occupied by an enemy
    ...
    bool IsEnemyAtPosition(Vector2Int gridPos, GameObject groundObject)
    {
        ...
        float checkRadius = gridManager.gridSize * 0.4f; // Slightly smaller than grid size
        Collider[] colliders = Physics.OverlapSphere(worldPos, checkRadius);
        
        foreach (Collider col in colliders)
        {
            // Check if this object is an enemy
            if (col.CompareTag("Enemy") || col.name.Contains("Enemy") || col.GetComponent<EnemyUnit>() != null)
            {
                if (enableDebugLogs)
                {
