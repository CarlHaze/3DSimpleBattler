...
public class GridMovementHighlighter : MonoBehaviour
{
    ...
    public float clickDebounceTime = 0.1f; // Prevent rapid clicking
    ...
    void HandleInput()
    {
        ...
        {
            // Prevent rapid clicking that could cause duplicate highlights
    ...
    void ClearMovementHighlights()
    {
        ...
        // Extra safety: also destroy any remaining highlights with our naming pattern
        GameObject[] allHighlights = GameObject.FindObjectsByType<GameObject>(FindObjectsSortMode.None);
        foreach (GameObject obj in allHighlights)
        {
            if (obj.name.StartsWith("MovementHighlight_"))
            {
                Destroy(obj);
            }
        }
