...
public class HeightAwarePathfinder : MonoBehaviour
{
    ...
    private bool CanMoveDirectly(Vector2Int from, Vector2Int to, GameObject groundObject)
    {
        ...
        // NEW: Check if we need to go over an obstacle
        if (RequiresJumpingOverObstacle(from, to, groundObject))
        {
            if (debugPathfinding)
            {
                Debug.Log($"Movement from {from} to {to} requires jumping over obstacle - forcing A* pathfinding");
            }
            return false; // Force A* pathfinding to include intermediate waypoints
        }
    ...
    /// <summary>
    /// Check if movement requires jumping over an obstacle (wall/elevated terrain)
    /// </summary>
    private bool RequiresJumpingOverObstacle(Vector2Int from, Vector2Int to, GameObject groundObject)
    {
        if (heightDetector == null) return false;
        
        // Get heights of start and end positions
        float fromHeight = heightDetector.GetGroundHeightAtGridPosition(from, groundObject);
        float toHeight = heightDetector.GetGroundHeightAtGridPosition(to, groundObject);
        
        // If they're at roughly the same height, check if there's an obstacle in between
        if (Mathf.Abs(toHeight - fromHeight) < 0.5f)
        {
            // Check intermediate positions between start and end
            ...
            for (int step = 1; step < steps; step++)
            {
                ...
                Vector2Int intermediate = new Vector2Int(
                    Mathf.RoundToInt(Mathf.Lerp(from.x, to.x, t)),
                    Mathf.RoundToInt(Mathf.Lerp(from.y, to.y, t))
                );
                
                float intermediateHeight = heightDetector.GetGroundHeightAtGridPosition(intermediate, groundObject);
                
                // If there's a significant height increase in the middle, we need to jump over it
                if (intermediateHeight > fromHeight + 0.5f && intermediateHeight > toHeight + 0.5f)
                {
                    return true;
                }
            }
